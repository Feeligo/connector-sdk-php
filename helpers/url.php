...
class FeeligoHelperUrl {
  ...
  public function url_for_entity_element($entity) {
    $path = $this->_pluralize($entity->type()) . '/' . $entity->id();
    return $this->url_for($path);
  }
  public function url_for_entity_collection($entity) {
    return $this->url_for($this->_pluralize($entity->type()));
  }
